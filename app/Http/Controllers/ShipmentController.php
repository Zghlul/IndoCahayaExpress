...
class ShipmentController extends Controller
{
    ...
    // Mapping layanan ke tabel rate (sama dengan OrderController)
    private $tableMap = [
        'PRIORITY'    => 'dhl_rate',
        'FedEx'       => 'fedex_rate',
    ...
    public function store(Request $request)
    {
        ...
        // 🔽 HITUNG MODAL DARI RATES
        $price = $request->price ?: 0;
        $modal = 0;
        if ($price > 0) {
            // Jika harga manual, tetap cari modal dari rates
            $modalResult = $this->getModalFromTable($request->service, $country_name, $chargeable_weight);
            $modal = $modalResult['modal'] ?? 0;
        } else {
            // Jika harga otomatis, cari modal dari rates
            $modalResult = $this->getModalFromTable($request->service, $country_name, $chargeable_weight);
            $modal = $modalResult['modal'] ?? 0;
        }
        ...
        DB::table('shipments')->insert([
            ...
            'charge_idr'         => $price,
            'modal'              => $modal, // 🔽 TAMBAHKAN KOLOM MODAL
    ...
    {
        // ... (kode calculateRate tetap sama seperti sebelumnya)
        // Tidak diubah, hanya copy dari file asli
        $request->validate([
